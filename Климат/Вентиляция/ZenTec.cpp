...
u8 cond [5];
u8 read[8]={0xF7, 0x03, 0x0B, 0xB8, 0x00, 0x04, 0xCC, 0x16};
u8 send[17]={0xF7, 0x10, 0x0B, 0xB8, 0x00, 0x04, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xCC, 0x16};
u8 write=0;
    ...
    if(senderId()!=exciterId())
        {
            ...
            {
                send[8]=0x01; send[12]=0x02;
            } else
            if(opt(0)%2==0)
            {
                send[8]=0x00; send[12]=0x02;
            }
            //Скорость
            if(opt(4)==1)
            {
...
V-ID/RS485
{
    ...
    {
        cond[0]=opt(4); cond[1]=opt(10)-10; cond[4]=transformspeed(opt(6));
        setStatus(V00, &cond);
    }
}