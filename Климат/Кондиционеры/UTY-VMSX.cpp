/*
{
  ...
  addItems:[
    {tag:"item", id:"%TARGET%", name:"NAME1", "sub-id":"%SUBID01%", "type":"conditioner", "t-min":"18", "t-delta":"12", "vane-ver":"0x00", "vane-hor":"0x00", "funs":"0x0E"},
        ...
        {tag:"item", id:"%TARGET%", name:"NAME5", "sub-id":"%SUBID05%", "type":"conditioner", "t-min":"18", "t-delta":"12", "vane-ver":"0x00", "vane-hor":"0x00", "funs":"0x0E"},
        {tag:"item", id:"%TARGET%", name:"NAME6", "sub-id":"%SUBID06%", "type":"conditioner", "t-min":"18", "t-delta":"12", "vane-ver":"0x00", "vane-hor":"0x00", "funs":"0x0E"},
  ],
  vars:[
        ...
        {name:"NAME6",type:"string",required:false,min:3,max:40,desc:"Имя кондиционера",defaultValue:"Кондиционер 6"},
        ...
        {type:"comment", text:"Режимы кондиционера 5:"},
        {name:"Fan5",  type:"checkbox", checked:false,desc:"Вентиляция"},
        {name:"Cool5", type:"checkbox", checked:false,desc:"Охлаждение"},
        {name:"Dry5",  type:"checkbox", checked:false,desc:"Осушение"},
        {name:"Heat5", type:"checkbox", checked:false,desc:"Нагрев"},
        {name:"Auto5", type:"checkbox", checked:false,desc:"Авто"},
        {type:"comment", text:"Режимы кондиционера 6:"},
        {name:"Fan6",  type:"checkbox", checked:false,desc:"Вентиляция"},
        {name:"Cool6", type:"checkbox", checked:false,desc:"Охлаждение"},
        {name:"Dry6",  type:"checkbox", checked:false,desc:"Осушение"},
        {name:"Heat6", type:"checkbox", checked:false,desc:"Нагрев"},
        {name:"Auto6", type:"checkbox", checked:false,desc:"Авто"},
  ]
}
*/
const u16 ID [] = {
...
u8 modes [] = {
    ...
    , #ifdef Fan5  3 #else 0xFF#endif
    , #ifdef Cool5 4 #else 0xFF#endif
    , #ifdef Dry5  2 #else 0xFF#endif
    , #ifdef Heat5 1 #else 0xFF#endif
    , #ifdef Auto5 0 #else 0xFF#endif
    , #ifdef Fan6  3 #else 0xFF#endif
    , #ifdef Cool6 4 #else 0xFF#endif
    , #ifdef Dry6  2 #else 0xFF#endif
    , #ifdef Heat6 1 #else 0xFF#endif
    , #ifdef Auto6 0 #else 0xFF#endif
};
const u8 Addr [] = {
    Address1
    ...
    #ifdef Address6, Address6 #else ,0#endif
};


u8 cond [5];
u16 Error [] = {0,0,0,0,0,0};
u8 counter=0;
u8 write=0;

void GetState() //Функция обратной связи(запрос состояния)
{
    ...
    if(counter<6) delayedCallMs(GetState,150);
    else{
        counter=0;
...
{
  delayedCallR(GetState,7);
  for(u8 i=0;i<6;++i) //Обработка режимов
  {
      for(u8 x=0;x<5;++x){
          for(u8 j=1;j<6;++j){
...
void Send()
{
    ...
    if(NeedSend)
    {
        for(u8 i=0;(i<6) && (breakpoint==0);++i)
        {
          ...
          else if(i==5)getStatus(Cond06,state);
          ...
          if((NeedSend>>i) & 1)
          {
            ...
            } else
            {
              ...
              if(NumSend==4) //ВКЛ-ВЫКЛ
              {
                  SendMas[5] = state[0]&1;
              }
              else if(NumSend==3) //Режим
              {
                  SendMas[3] = 1;
                  SendMas[5] = modes[1+(state[0]>>4) + 5*i];
              }
              else if(NumSend==2) //Скорость
              {
                  SendMas[3] = 2;
                  SendMas[5] = state[4];
              }
              else if(NumSend==1) //Температура
              {
                  SendMas[3] = 4;
              ...
              else {cancelDelayedCall(Write); delayedCall(Write, 5);}
            }
            }
        }
    }
...
V-ID/RS485{
    ...
    #ifdef Cond05, [Cond05.0] #endif
    #ifdef Cond06, [Cond06.0] #endif
    };
    ...
    #ifdef Cond05, [Cond05.1] #endif
    #ifdef Cond06, [Cond06.1] #endif
    };
