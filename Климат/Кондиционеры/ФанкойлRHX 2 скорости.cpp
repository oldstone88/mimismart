/*
{
	desc:"Фанкойлы сухой контакт 2 скорости",
	tag:"import-script",
	selectArea:true,
...
void checkQ()
{
	...
	{
		if(NV-NT+100<=100-D2) {setStatus(C1,0); setStatus (C2, 1);} else
		if(NV-NT+100<=100-D1) {setStatus(C2,0); setStatus (C1, 1);} else
		if((NV-NT+100>=100) && ([C1.0]!=0 || [C2.0]!=0)){setStatus(C1,0); setStatus (C2, 0);}
	} else
	if ([V1.0]==33) //Нагрев
	{
		if(NV-NT+100>=100+D2) {setStatus(C1,0); setStatus (C2, 1);} else
...
if ([V1.0]%2!=0) //Если кондиционер включен
	...
	if( [V1.4]==1 ) {cancelDelayedCall(checkQ); setStatus(C2,0); setStatus (C1, 1);} else //Ручная первая скорость
	if( [V1.4]==2 ) {cancelDelayedCall(checkQ); setStatus(C1,0); setStatus (C2, 1);} else //Ручная вторая скорость
	if( [V1.4]==0 ) {cancelDelayedCall(checkQ); setStatus(C2,0); setStatus (C1, 1); checkQ(); delayedCallR(checkQ, TIME); } //Режим автомат

	}
		else {cancelDelayedCall(checkQ); cancelDelayedCall(checkR); setStatus(K1,0); setStatus(K2,0); setStatus(C1,0); setStatus(C2,0);} // Выключение кондиционера
}
