...
V-ID/RS485{
    ...
    if(optl==11 && opt(1)==0x03){
    	if(opt(0)==1){getStatus(Cond01, &cond);}
        else if(opt(0)==2){getStatus(Cond02, &cond);}
        else if(opt(0)==3){getStatus(Cond03, &cond);}
        else if(opt(0)==4){getStatus(Cond04, &cond);}
        cond[0]&=1;
        cond[0]=cond[0] | (backmode(opt(6))<<4); //Режим
