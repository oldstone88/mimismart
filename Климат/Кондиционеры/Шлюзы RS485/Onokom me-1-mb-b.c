/*
{
  ...
  addItems:[
    ...
    {tag:"item", id:"%TARGET%", name:"Кондиционер 6", "sub-id":"%SUBID11%", "type":"conditioner", "t-min":"16", "t-delta":"16", "vane-ver":"0x00", "vane-hor":"0x00", "funs":"0x0F", "modes":"0x1F"},
    {tag:"item", id:"%TARGET%", name:"Кондиционер 7", "sub-id":"%SUBID13%", "type":"conditioner", "t-min":"16", "t-delta":"16", "vane-ver":"0x00", "vane-hor":"0x00", "funs":"0x0F", "modes":"0x1F"},
  ],
  vars:[
        ...
        {name:"MBS05",type:"number",min:"1",max:"127",required:true,defaultValue:"6",desc:"Адрес Modbus"},
        {name:"MBS06",type:"number",min:"1",max:"127",required:true,defaultValue:"7",desc:"Адрес Modbus"},
    ...
    if(NeedSend){
        for(u8 i=0; i<7 && (breakpoint==0); ++i){
            if((NeedSend>>i)&1){
                ...
                if(i==6){getStatus(COND06, cond);}

                if(SendNext == 0) SendNext = 3;
                // ВКЛ-ВЫКЛ + Режим
                else if(SendNext == 3){
                    // Указываем адрес отправки
                    writecond[0] = MBSAD[i];
                    // Указываем регистр
                    writecond[3] = 0x02;
                    writecond[4] = 0x00;
                    // Считываем режим с кондиционера
                    if((cond[0]%2)==0) writecond[5] = 0; // Выключен
                    else if(cond[0]==1) writecond[5] = 5; // Вентиляция
                    else if(cond[0]==17) writecond[5] = 2; // Холод
                    else if(cond[0]==33) writecond[5] = 4; // Сушка
                    else if(cond[0]==49) writecond[5] = 1; // Нагрев
                    else if(cond[0]==65) writecond[5] = 3; // Авто
                }
                // Температура
                else if(SendNext == 2){
                    writecond[0] = MBSAD[i];
...
void synccond(){
    ...
    else if(count==5) setStatus(COND05, &cond);
    else if(count==6) setStatus(COND06, &cond);
}
