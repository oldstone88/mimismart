...
void settings(){
    ...
    u8 condstatus[15]={Address, 0x10, modbusregister>>8, modbusregister, 0x00, 0x03, 0x06, 0x00, setTemp, 0x00, mode, 0x00, speed, 0xCC, 0x16};
...
void readStatus(){
    ...
    u8 readcond[8]={Address, 0x03, 0x00, modbusregister, 0x00, 0x03, 0xCC, 0x16};
...
void synccond(){
    ...
    // Установка статуса
    if(readcond==0) { cond[0]|=(COND00&1); setStatus(COND00, &cond);}
    else if(readcond==1) {cond[0]|=(COND01&1); setStatus(COND01, &cond);}
    else if(readcond==2) {cond[0]|=(COND02&1); setStatus(COND02, &cond);}
    else if(readcond==3) {cond[0]|=(COND03&1); setStatus(COND03, &cond);}
}

V-ID/RS485
{
    // Проверка ВКЛ-ВЫКЛ
    if(opt(0)==Address && opt(1)==0x01 && optl==6){
        u8 cond[5]={0, 0, 0, 0 ,0};
