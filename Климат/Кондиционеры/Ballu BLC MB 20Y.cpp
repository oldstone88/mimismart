...
u8 breakpoint = 0;
u8 writehold[15]={0x01, 0x10, 0x00, 0x00, 0x00, 0x03, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xCC, 0x16};
u8 writecoil[8]={0x01, 0x05, 0x00, 0x00, 0x00, 0x00, 0xCC, 0x16};
u8 readhold[8]={0x01, 0x03, 0x00, 0x00, 0x00, 0x03, 0xCC, 0x16};
u8 readcoil[8]={0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 0xCC, 0x16};
u8 count=0;
    ...
    if(NeedSend){
        for(u8 i=0, (i<4) && (breakpoint==0), ++i){
            if( (NeedSend>>i)&1 ){
                ...
                // Назначаем адрес блока
                u16 address = (20*i)+1;
                writecoil[2]=address>>8;
                writeсoil[3]=address;
                ++address; // Добалсяем 1, т. к нужное нам начинается в 2м(3) регистре
                writehold[2]=address>>8;
                writehold[3]=address;
                if(state[0]%2==1) writecoil[4]=0xFF;
                else writecoil[4]=0x00;
                setStatus(RS485, writecoil);
                writehold[10]=state[1]+16; // Температура
...
void onOffcheker(){
    if(write==0){
        u16 address = (20*count)+1;
        readcoil[2]=address>>8;
        readcoil[3]=address;
...
V-ID/s:5{
    if(write==0){
        u16 address = (20*count)+2;
        readhold[2]=address>>8;
        readhold[3]=address;
...
V-ID/RS485{
    ...
    if(optl==11 && opt(1)==0x03){
        u8 cond[5]={0, 0, 0, 0, 0};
        cond[0]|=backmode(opt(4))<<4; //Режим
        u8 temperature=opt(6)-16;
        ...
        if(count==1){setStatus(Cond01, &cond);}
        else if(count==2){setStatus(Cond02, &cond);}
        else if(count==3){setStatus(Cond03, &cond);}
        else if(count==0){setStatus(Cond04, &cond);}
    }
    else if(optl==6 && opt(1)==0x01){
        u8 OnOff=opt(3);
        u8 cond[5]={0, 0, 0, 0, 0};
        if(count==1){getStatus(Cond01, &cond); cond[0]|=OnOff; setStatus(Cond01, &cond);}
        else if(count==2){getStatus(Cond02, &cond); cond[0]|=OnOff; setStatus(Cond02, &cond);}
        else if(count==3){getStatus(Cond03, &cond); cond[0]|=OnOff; setStatus(Cond03, &cond);}
        else if(count==0){getStatus(Cond04, &cond); cond[0]|=OnOff; setStatus(Cond04, &cond);}
    }
}