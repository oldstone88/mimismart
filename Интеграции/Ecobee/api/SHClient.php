    ...
    public function setDeviceState($id, $subid, $values){
        ...
        if($deviceType != ""){
            if($length == 1){
                ...
                // var_dump($index);
                $currentState = array("powerSwitch" => 0, "temperature" => 0, "vane" => 0, "air-fan" => 0,"wide_vane" => 0, "mode" => 0);;
                // var_dump($currentState);
                if(array_key_exists("powerSwitch", $currentState)) $state[0] = ((int)$currentState["powerSwitch"] & 0xf0) | (int)$values["powerSwitch"];
                ...
                $state[0] = ($state[0] & 0xf)|((int)$values["mode"] << 4);
