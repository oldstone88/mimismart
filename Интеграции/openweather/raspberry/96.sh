...
$city = $data->name;
$temp = $data->main->temp;
$tempokr = substr($temp, strpos($temp,'.')+1);
$temp_like = $data->main->feels_like;
$temp_likeokr = substr($temp_like, strpos($temp_like,'.')+1);
$humidity = $data->main->humidity;
$humidityokr = substr($humidity, strpos($humidity,'.')+1);
$speed = $data->wind->speed;
$speedokr = substr($speed, strpos($speed,'.')+1);
...
if(isset($dstId2) and isset($dstSubId2)){
    ...
    $buffer = pack("SSCCCCScc", 1, $IdSensor, 5, 244, 67, $SidSensor, 2,$speedokr,$speed); //126, data - 126 - дробная часть, data-целая
...
if(isset($dstId3) and isset($dstSubId3)){
    ...
    $SidSensor = $dstSubId3;
    $socket = fsockopen("udp://$pIP", 55555, $errno, $errstr);
    $buffer = pack("SSCCCCScc", 1, $IdSensor, 5, 244, 67, $SidSensor, 2,$humidityokr,$humidity); //126, data - 126 - дробная часть, data-целая
    fwrite($socket, $buffer);
    fclose($socket);
	}
...
if(isset($dstId4) and isset($dstSubId4)){
    ...
    $SidSensor = $dstSubId4;
    $socket = fsockopen("udp://$pIP", 55555, $errno, $errstr);
    $buffer = pack("SSCCCCScc", 1, $IdSensor, 5, 244, 67, $SidSensor, 2,$temp_likeokr,$temp_like); //126, data - 126 - дробная часть, data-целая
    fwrite($socket, $buffer);
    fclose($socket);
	}
...
if(isset($dstId5) and isset($dstSubId5)){
    ...
    $SidSensor = $dstSubId5;
    $socket = fsockopen("udp://$pIP", 55555, $errno, $errstr);
    $buffer = pack("SSCCCCScc", 1, $IdSensor, 5, 244, 67, $SidSensor, 2,$tempokr,$temp); //126, data - 126 - дробная часть, data-целая
    fwrite($socket, $buffer);
    fclose($socket);
	}
