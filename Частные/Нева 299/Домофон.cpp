...
if(opt(0) == 1)
    {
        ...
        if(state[0]==1) {getStatus(MRMR0, &i0);
                setStatus(MRMR0,{1, state[1], 0, "http://192.168.1.125/Ding.mp3"});
                delayedCall(V0, 2);}
        #ifdef MRMR1
        if(state[0]==1){getStatus(MRMR1, &i1);
                setStatus(MRMR1,{1, state[1], 0, "http://192.168.1.125/Ding.mp3"});
                delayedCall(V1, 2);}
        #endif
        #ifdef MRMR2
        if(state[0]==1){getStatus(MRMR2, &i2);
                setStatus(MRMR2,{1, state[1], 0, "http://192.168.1.125/Ding.mp3"});
                delayedCall(V2, 2);}
        #endif
        #ifdef MRMR3
        if(state[0]==1){getStatus(MRMR3, &i3);
                setStatus(MRMR3,{1, state[1], 0, "http://192.168.1.125/Ding.mp3"});
                delayedCall(V3, 2);}
        #endif
        #ifdef MRMR4
        if(state[0]==1) {getStatus(MRMR4, &i4);
                setStatus(MRMR4,{1, state[1], 0, "http://192.168.1.125/Ding.mp3"});
                delayedCall(V4, 2);}
        #endif
