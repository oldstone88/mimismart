...
void NAGREV()
{
  ...
  {
    if(k[0]!=49 || k[1]!=[AUTO.1] || k[4]!=0) {k[0]=49; k[1]=[AUTO.1]; k[4]=0; setStatus(COND, &k);}
    #ifdef EL00 if( ([EL00.0]&1)==0 ) setStatus(EL00, 1); #endif //&& timer>=(TIME*12) - добавить в условие если потребуется таймер
    #ifdef EL01 if( ([EL01.0]&1)==0 ) setStatus(EL01, 1); #endif //&& timer>=(TIME*12) - добавить в условие если потребуется таймер
    #ifdef EL02 if( ([EL02.0]&1)==0 ) setStatus(EL02, 1); #endif //&& timer>=(TIME*12) - добавить в условие если потребуется таймер
...
void HOLOD()
{
  i=2;
  buff=-7;
  if( transform([STREET.1])>buff ) //на улице больше -7
  {
  ...
  if([AUTO.0]%2!=0)
    {
      ...
      if(tempAuto+180-GIST > tempK) //Нагрев
        {
          //if(timer<=TIME*12)++timer; //Расскоментировать если нужен таймер
          NAGREV();
        } else
