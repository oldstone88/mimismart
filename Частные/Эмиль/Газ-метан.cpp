...
V-ID/s:300
{
    if([METAN.1]>40 && ([V-ADDR.0]&1)==0) {setStatus(V-ADDR, 0); setStatus(2047:32, {8, "Высокий уровень ГАЗА!"});}
}

V-ID/s:5
{
    if([METAN.1]>40 && ([V-ADDR.0]&1)==1) {if(timer<120) ++timer; if(timer>=120) {setStatus(V-ADDR, 0); setStatus(2047:32, {8, "Утечка ГАЗА!!!"});} } else timer=0;
}
