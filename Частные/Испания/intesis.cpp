...
void GetState(){
    ...
    if(counter<12) delayedCallMs(GetState,150);
    else{
        counter=0;
...
void Send() {
    ...
    u8 breakpoint = 0;
    if(NeedSend){
        for(u8 i=0;(i<12) && (breakpoint==0);++i){
            u8 state [5];
...
V-ID/COND00, COND01, COND02, COND03, COND04, COND05, COND06, COND07, COND08, COND09, COND10, COND11
{
